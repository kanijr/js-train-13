...
function manipulateUrl(url) {
  ...
  delete newUrl.searchParams.delete("oldParam");
  return newUrl;
}
...
function searchParamsURL(url) {
  ...
  // Повертаємо словник
  let newUrl = new URL(url);
  let params = newUrl.searchParams.entries();
  let map = new Map();
...
function checkURLParameters(url, params) {
  ...
  let obj = Object();
  for (let param of params) {
    obj[param] = newUrl.searchParams.has(param);
  }
  return obj;
}
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  let urlObj = new URL(url);
  let arr = [...urlObj.searchParams].sort();
  console.log(arr);
  urlObj.search = "";
